...
import { 
    Home, Clock, Activity, Users, Settings, FileText, Globe, Stethoscope, BriefcaseMedical, 
    Calendar, LogOut, User, MapPin, Zap, MessageCircle, Star, TrendingUp, BarChart, 
    ClipboardList, Briefcase, ChevronDown, CheckCircle, XCircle 
} from 'lucide-react';
...
const translations = {
    en: {
        ...
        alerts: {
            ...
            alert1Detail: "2 patients flagged today based on BP > 160/100 mmHg. Review charts immediately.",
    ...
    hi: {
        ...
        alerts: {
            ...
            alert1Detail: "2 रोगी आज बीपी > 160/100 mmHg के आधार पर फ़्लैग किए गए। तुरंत चार्ट की समीक्षा करें।",
    ...
    bn: {
        ...
        alerts: {
            ...
            alert1Detail: "BP > 160/100 mmHg এর ভিত্তিতে আজ 2 জন রোগীকে পতাকাঙ্কিত করা হয়েছে। অবিলম্বে চার্ট পর্যালোচনা করুন।",
...
const TopNavDoctor = ({ t, language, setLanguage, setRoute, currentSeating, setCurrentSeating }) => {
                ...
                <div className="flex items-center space-x-2 bg-indigo-600 p-2 rounded-lg shadow-inner">
                    ...
                    >
                        {seatingOptions.map((opt) => (
                            <option key={opt.key} value={opt.key} className="text-black bg-white">
                                {opt.label}
                            </option>
                        ))}
                    </select>
...
const DoctorSidebar = ({ t, setRoute }) => {
    const navItems = useMemo(() => ([
        ...
        { name: 'clinicalAnalytics', icon: TrendingUp, route: 'clinicalAnalytics' },
    ...
    return (
        <nav className="space-y-2">
            <h2 className="text-lg font-bold text-gray-700 mb-4 border-b pb-2">{t('doctorSpecialty')}</h2>
            {navItems.map((item) => (
                <button 
                    key={item.name} 
...
const DoctorAlertItem = ({ t, titleKey, detailKey, icon: Icon, color }) => (
    <div className="p-3 bg-white rounded-lg shadow-sm border-l-4" style={{ borderColor: color }}>
        ...
        <button className="mt-2 text-xs text-indigo-500 hover:underline">Review</button>
    </div>
);

const DoctorRightPanel = ({ t }) => {
    const alerts = useMemo(() => [
        { titleKey: 'alert1', detailKey: 'alert1Detail', icon: Zap, color: '#ef4444' }, // Red for urgency
        { titleKey: 'alert2', detailKey: 'alert2Detail', icon: MessageCircle, color: '#3b82f6' }, // Blue for communication
        { titleKey: 'alert3', detailKey: 'alert3Detail', icon: ClipboardList, color: '#f59e0b' }, // Amber for tasks
    ], []);
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-white p-4 rounded-xl shadow border border-gray-200 flex flex-col justify-center">
                    <p className="text-3xl font-extrabold text-gray-900 flex items-center">
                        {t('doctorRating')} <Star className="w-5 h-5 ml-1 text-yellow-500 fill-yellow-500"/>
                    </p>
            ...
            <div className="p-4 bg-indigo-50 rounded-xl shadow border border-indigo-200">
                <h3 className="text-lg font-bold text-indigo-800 mb-3">{t('alerts.title')}</h3>
...
const DoctorDashboardContent = ({ t }) => (
    <div className="space-y-8">
        ...
        <p className="text-gray-600">Quick overview of daily statistics and critical patient alerts.</p>

        {/* Mock Charts */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white p-6 rounded-xl shadow-lg h-64 flex items-center justify-center">
                <p className="text-indigo-500 font-semibold">Mock: Daily Consultations (Bar Chart)</p>
            </div>
            <div className="bg-white p-6 rounded-xl shadow-lg h-64 flex items-center justify-center">
                <p className="text-indigo-500 font-semibold">Mock: Patient Demographics (Pie Chart)</p>
            </div>
        </div>
    </div>
);
...
const DoctorScheduleContent = ({ t }) => (
    <div className="space-y-8">
        ...
        <p className="text-gray-600">Manage appointments and check patient status in the queue.</p>
        
        <div className="bg-white p-6 rounded-xl shadow-lg">
            <h3 className="text-xl font-semibold mb-4 text-indigo-700">Today: Tuesday, Oct 22</h3>
            <div className="space-y-4">
                <div className="flex justify-between items-center p-3 border-b">
                    <span className="font-medium">10:00 AM - Asha Devi (P011)</span>
                    <span className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded-full">In Queue</span>
                </div>
                <div className="flex justify-between items-center p-3 border-b">
                    <span className="font-medium">10:30 AM - Vinay Gupta (P012)</span>
                    <span className="text-xs bg-yellow-100 text-yellow-700 px-2 py-1 rounded-full">Consulting</span>
                </div>
            ...
            <button className="mt-4 text-white bg-indigo-600 px-4 py-2 rounded-full hover:bg-indigo-700">View Full Schedule</button>
        </div>
    </div>
);
...
const DoctorRecordsContent = ({ t }) => (
    <div className="space-y-8">
        ...
        <p className="text-gray-600">Search patient history, diagnostic reports, and notes.</p>
        <input 
            type="text" 
...
const DoctorAnalyticsContent = ({ t }) => (
    <div className="space-y-8">
        ...
        <p className="text-gray-600">Review clinical efficiency and outcome metrics.</p>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-green-100 p-4 rounded-xl shadow">Outcome Success Rate: 85%</div>
            <div className="bg-yellow-100 p-4 rounded-xl shadow">Average Consultation Time: 18 min</div>
            <div className="bg-blue-100 p-4 rounded-xl shadow">E-Prescription Adoption: 98%</div>
        </div>
    </div>
);
...
const DoctorCEContent = ({ t }) => (
    <div className="space-y-8">
        ...
        <p className="text-gray-600">Access latest research, guidelines, and continuing education modules.</p>
        <div className="bg-white p-6 rounded-xl shadow-lg">
            <p className="font-semibold text-indigo-700">New Oncology Guidelines (2025)</p>
            <p className="text-sm text-gray-500">Video Module available now. Complete by Nov 30.</p>
            <button className="mt-3 text-sm text-white bg-indigo-500 px-3 py-1 rounded-full hover:bg-indigo-600">Start Module</button>
        </div>
    </div>
);
...
const DoctorFallbackContent = ({ t, route }) => {
    ...
    return (
        <div className="text-center p-10 bg-white rounded-xl shadow-lg border-2 border-dashed border-gray-300">
            ...
            <p className="text-gray-600">Content for this professional area is currently under development.</p>
        </div>
    );
}
